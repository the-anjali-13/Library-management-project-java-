...
class Addbook extends JFrame implements ActionListener
{
    ...
    Addbook()
    {
        ...
        b1 = new JButton("add confirm");
        b1.setBounds(200,480,100,40);

        b2 = new JButton("back");
        b2.setBounds(400,580,80,30);

        add(l1); add(l2); add(l3); add(l4); add(l5);add(l6);add(t1);add(t2);add(t3);add(t4);add(t5);add(b1);add(b2);
    ...
    public void actionPerformed(ActionEvent e)
    {
        if(e.getSource()==b1)
        {
            ...
            try
            {
                ...
                String Query ="insert into addbook(callno,bookname,author,publisher,quantity)values(?,?,?,?,?)";
                PreparedStatement pt = con.prepareStatement(Query);

                pt.setString(1,callno);
                pt.setString(2,bookname);
                ...
                int row = pt.executeUpdate();
                        ...
                        {
                            JOptionPane.showMessageDialog(this, "OOPS! ERROR TO ADD BOOK");
